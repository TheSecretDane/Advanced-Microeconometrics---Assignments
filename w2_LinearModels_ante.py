...
def estimate(y: np.ndarray, x: np.ndarray, transform='', N=None, T=None) -> dict:
    ...
    p_val = (1 - stats.norm.cdf(np.abs(t_values), loc=0, scale=1)) * 2

    names = ['b_hat', 'se', 'sigma', 't_values', "resid", 'R2', 'cov', 'p_val']
    results = [b_hat, se, sigma, t_values, resid, R2, cov, p_val]
...
def variance( 
    ...
    if transform in ('', 're', 'fd'):
          ...
          sigma = SSR / (N * T - N - K)
    elif transform.lower() == '2fe':
          sigma = SSR / (N * T - (N + T - 1) - K)
