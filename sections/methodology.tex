...
\end{align*}
where $y_{it} = \ln Y_{it}$ and so on. Suppose that $\ln A_{it}$ can be decomposed into a time-invariant firm-specific effects, $\alpha_i$ and add an idiosyncratic error, $u_{it}$, i.e. $v_{it} = \alpha_i + u_{it}$. The model is then given by,
\begin{align*}
    v_{it} &= c_i + u_{it}, \\
    y_{it} &= c_i + \beta_K k_{it} + \beta_L l_{it} + u_{it}
\end{align*}
